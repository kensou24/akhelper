...
void UIDemo7::initForm()
{
  ...
  QStringList qss;
  qss.append(
             "QLabel#labTitle{background-color:qlineargradient(spread:pad, x1:0, y1:0, x2:1, y2:0, stop:0 #2AA2DA, stop:1 #12B5C9);color:#F0F0F0;}");
...
void UIDemo7::initNavLeft()
{
  ...
  tbtnLeft << ui->tbtn1 << ui->tbtn2 << ui->tbtn3 << ui->tbtn4 << ui->tbtn5 <<
    ui->tbtn6;

  int count = tbtnLeft.count();

  for (int i = 0; i < count; i++) {
    pixNormalLeft.append(IconHelper::Instance()->getPixmap(normalTextColor,
                                                           QChar(pixCharLeft.at(i)),
                                                           size, pixWidth,
                                                           pixHeight));
    pixDarkLeft.append(IconHelper::Instance()->getPixmap(selectTextColor,
                                                         QChar(pixCharLeft.at(
                                                                 i)),
                                                         size, pixWidth,
                                                         pixHeight));
    tbtnLeft.at(i)->setIcon(QIcon(pixNormalLeft.at(i)));
    ...
    connect(tbtnLeft.at(i), SIGNAL(clicked(bool)), this,
            SLOT(buttonClickedLeft()));
  }
...
void UIDemo7::initNavTop()
{
  ...
  for (int i = 0; i < count; i++) {
    pixNormalTop.append(IconHelper::Instance()->getPixmap(normalTextColor,
                                                          QChar(pixCharTop.at(i)),
                                                          size, pixWidth,
                                                          pixHeight));
    pixDarkTop.append(IconHelper::Instance()->getPixmap(selectTextColor,
                                                        QChar(pixCharTop.at(i)),
                                                        size, pixWidth,
                                                        pixHeight));
    tbtnTop.at(i)->setIcon(QIcon(pixNormalTop.at(i)));
