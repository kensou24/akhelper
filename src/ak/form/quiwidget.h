...
public:
  ...
  // 判断IP地址及端口是否在线
  static bool ipLive(const QString& ip,
                     int            port,
                     int            timeout = 1000);
  ...
  // 弹出输入框
  static QVariantMap showInputListBox(bool             & ok,
                                      const QString    & title,
...
public Q_SLOTS:
  ...
  // 设置部件图标
  void setIcon(QUIWidget::Widget widget,
               QChar             str,
               quint32           size = 9);
  void setIconMain(QChar   str,
                   quint32 size = 9);
  ...
  // 设置部件是否可见
  void setVisible(QUIWidget::Widget widget,
                  bool              visible = true);
...
// 扩展的支持多条数据的 弹出输入框类
class QUIInputListBox : public QDialog {
  Q_OBJECT

public:

  explicit QUIInputListBox(QWidget *parent = 0);
  ~QUIInputListBox();

  static QUIInputListBox* Instance()
  {
    static QMutex mutex;
...
private:
  ...
  QList<QLabel *>labInfos;
  QList<QLineEdit *>txtValues;
  QComboBox *cboxValue;
  QHBoxLayout *lay;
  QSpacerItem *horizontalSpacer;
  QPushButton *btnOk;
...
private slots:
  ...
  void initForm();                               // 初始化窗体
  void checkSec();                               // 校验倒计时
  void initContent(const QStringList& contents); // 初始化输入框
...
public:
  ...
  void    setIcon(QLabel *lab,
                  QChar   c,
                  quint32 size = 9);
  void    setIcon(QAbstractButton *btn,
                  QChar            c,
                  quint32          size = 9);
  QPixmap getPixmap(QChar c);
  ...
  // 根据按钮获取该按钮对应的图标
  QPixmap getPixmap(QToolButton *btn,
                    bool         normal);
