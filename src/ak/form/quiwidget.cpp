...
void QUIWidget::writeInfo(const QString& info, const QString& filePath)
{
  QString fileName = QString("%1/%2/%3_runinfo_%4.txt").arg(QUIWidget::appPath())
                     .arg(filePath).arg(QUIWidget::appName()).arg(
                            ...
                            QString      & highColor)
{
  ...
  if (indexNormalColorStart >= 0) {
    normalColorStart = str.mid(
      indexNormalColorStart + flagNormalColorStart.length(),
      7);
  }
...
# endif // if _MSC_VER
  ...
  if (codec == Q_NULLPTR) {
    qDebug() << "can not find codec for utf-8";
  }
  QTextCodec::setCodecForLocale(codec);
                                  ...
                                  const QString& sec)
{
  ...
  p.close();
#else // ifdef Q_OS_WIN
  QString cmd = QString("date %1%2%3%4%5.%6").arg(month).arg(day).arg(hour).arg(
    min).arg(year).arg(sec);
  system(cmd.toLatin1());
  system("hwclock -w");
...
bool QUIWidget::isTel(const QString& tel)
{
  ...
  if (!tel.startsWith("13") && !tel.startsWith("14") && !tel.startsWith("15") &&
      !tel.startsWith("18")) {
    return false;
  }
...
QVariantMap QUIWidget::showInputListBox(bool             & ok,
                                        const QString    & title,
                                        ...
                                        bool               pwd)
{
  QUIInputListBox input;

  input.setParameter(title, contents, type, closeSec, defaultValue, pwd);
  ok = input.exec();
  return input.getValue();
}
...
void QUIMessageBox::initControl()
{
  ...
  labIcoMain->setStyleSheet(QString::fromUtf8(
                              "border-image: url(:/image/msg_info.png);"));

  horizontalLayout1->addWidget(labIcoMain);

  horizontalSpacer1 = new QSpacerItem(5,
                                      0,
                                      QSizePolicy::Minimum,
                                      QSizePolicy::Minimum);
  ...
  horizontalSpacer2 = new QSpacerItem(40,
                                      20,
  ...
  QIcon icon;
  icon.addFile(QString::fromUtf8(":/image/btn_ok.png"),
               QSize(),
               QIcon::Normal,
               QIcon::Off);
  btnOk->setIcon(icon);
  ...
  QIcon icon1;
  icon1.addFile(QString::fromUtf8(":/image/btn_close.png"),
                QSize(),
                QIcon::Normal,
                QIcon::Off);
  ...
  connect(this->btnOk, SIGNAL(clicked()), this,
...
{
  IconHelper::Instance()->setIcon(labIco,
                                  QUIConfig::IconMain,
                                  QUIConfig::FontSize + 2);
  IconHelper::Instance()->setIcon(btnMenu_Close,
                                  QUIConfig::IconClose,
                                  QUIConfig::FontSize);
...
QPixmap IconHelper::getPixmap(QChar c) {
  return getPixmap(les::iconProperty::normalTextColor,
                   c,
                   les::iconProperty::size,
                   les::iconProperty::pixWidth,
                   les::iconProperty::pixHeight);
}
                          ...
                          const QString& darkTextColor)
{
  ...
  if (type == "top") {
    strBorder = QString(
      "border-width:%1px 0px 0px 0px;padding:%1px %2px %2px %2px;")
                .arg(borderWidth).arg(borderWidth * 2);
  } else if (type == "right") {
    strBorder = QString(
      "border-width:0px %1px 0px 0px;padding:%2px %1px %2px %2px;")
                .arg(borderWidth).arg(borderWidth * 2);
  } else if (type == "bottom") {
    strBorder = QString(
      "border-width:0px 0px %1px 0px;padding:%2px %2px %1px %2px;")
                .arg(borderWidth).arg(borderWidth * 2);
  } else if (type == "left") {
    strBorder = QString(
      "border-width:0px 0px 0px %1px;padding:%2px %2px %2px %1px;")
                .arg(borderWidth).arg(borderWidth * 2);
  }
  ...
  qss.append(QString("QWidget[flag=\"%1\"] QAbstractButton:hover,"
             ...
             .arg(type).arg(strBorder).arg(borderColor).arg(darkTextColor).arg(
               darkBgColor));
                          ...
                          const QString     & darkTextColor)
{
  ...
  qss.append(QString("QWidget#%1{background:%2;}").arg(widget->objectName()).arg(
               normalBgColor));
  ...
  qss.append(QString(
               "QToolButton:hover,QToolButton:pressed,QToolButton:checked{background-color:%1;color:%2;}")
...
QUIInputListBox *QUIInputListBox::self = 0;
QUIInputListBox::QUIInputListBox(QWidget *parent) : QDialog(parent)
{
  this->initControl();
  this->initForm();
}

QUIInputListBox::~QUIInputListBox()
{
  delete widgetMain;
}

void QUIInputListBox::initControl()
{
  this->setObjectName(QString::fromUtf8("QUIInputListBox"));
  ...
  verticalLayout3->addLayout(gridLayout);
...
void QUIInputListBox::initContent(const QStringList& contents)
{
  bool find = false;
  ...
  foreach(QString content, contents) {
    for (int i = 0; i < labInfos.size(); i++) {
      QLabel *labInfo = labInfos[i];

      if (labInfo->text().compare(content) == 0) {
        find = true;
      }
      ...
      setTabOrder(txtValues[i], btnOk);
    }

    int count = labInfos.size();

    if (!find) {
      QLabel *labInfo = new QLabel(content, frame);

      labInfo->setObjectName(QString::fromUtf8("labInfo"));
      labInfo->setScaledContents(false);
      labInfo->setWordWrap(true);

      gridLayout->addWidget(labInfo, count, 0);


      QLineEdit *txtValue = new QLineEdit(frame);
      txtValue->setObjectName(QString::fromUtf8("txtValue"));
      gridLayout->addWidget(txtValue, count, 1);

      labInfos.append(labInfo);
...
void QUIInputListBox::setParameter(const QString& title,
                                   const QStringList& contents,
                                   int type, int closeSec,
                                   QString defaultValue, bool pwd)
{
  this->closeSec   = closeSec;
  this->currentSec = 0;
...
QVariantMap QUIInputListBox::getValue() const
{
  return this->values;
}

void QUIInputListBox::closeEvent(QCloseEvent *)
{
  closeSec   = 0;
  currentSec = 0;
}

bool QUIInputListBox::eventFilter(QObject *obj, QEvent *evt)
{
  static QPoint mousePoint;
  static bool   mousePressed = false;
...
void QUIInputListBox::on_btnOk_clicked()
{
  this->values.clear();

  for (int i =  0; i  < txtValues.size(); i++) {
    QLineEdit *txtValue = txtValues[i];

    if (txtValue->isVisible()) {
      values[labInfos[i]->text()] = (txtValue->text());
    } else if (this->cboxValue->isVisible()) {
      values[labInfos[i]->text()] = (this->cboxValue->currentText());
    }
  }
...
void QUIInputListBox::on_btnMenu_Close_clicked()
{
  done(QMessageBox::Cancel);
  close();
}

void QUIInputListBox::setIconMain(QChar str, quint32 size)
{
  IconHelper::Instance()->setIcon(this->labIco, str, size);
}
